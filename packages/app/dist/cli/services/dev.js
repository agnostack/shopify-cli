...
import { getURLsData, generateFrontendURL, shouldOrPromptUpdateURLs, startTunnelPlugin, updateURLsData, } from './dev/urls.js';
...
async function dev(options) {
    ...
    const { storeFqdn, remoteApp, remoteAppUpdated, updateURLsData: cachedUpdateURLsData, useCloudflareTunnels, } = await ensureDevContext(options, token);
    ...
    const appProxySubPath = backendConfig?.configuration.appProxy?.subPath ?? frontendConfig?.configuration.appProxy?.subPath;
