...
export async function updateURLsData(data, apiKey, token) {
    const variables = { apiKey, ...data };
...
export async function getURLsData(apiKey, token) {
...
export async function shouldOrPromptUpdateURLs(options) {
    ...
    if (options.cachedUpdateURLs === undefined) {
        const response = await updateURLsPrompt(options.currentURLsData.applicationUrl, options.currentURLsData.redirectUrlWhitelist, options.currentURLsData.appProxy);
        let newUpdateURLsData;
...
export function validatePartnerAppUpdate(data) {
    ...
    data.redirectUrlWhitelist.forEach((url) => {
        if (!isValidURL(url))
            throw new AbortError(`Invalid redirection URLs: ${data.redirectUrlWhitelist}`, 'Valid format: "https://example.com/callback1,https://example.com/callback2"');
    });
