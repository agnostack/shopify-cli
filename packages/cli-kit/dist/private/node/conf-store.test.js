import { cacheRetrieveOrRepopulate, getSession, removeSession, setSession } from './conf-store.js';
...
describe('getSession', () => {
    test('returns the content of the SessionStore key', async () => {
        await inTemporaryDirectory(async (cwd) => {
            // Given
            const config = new LocalStorage({ cwd });
...
describe('cacheRetrieveOrRepopulate', () => {
    test('returns the cached contents when they exist', async () => {
        await inTemporaryDirectory(async (cwd) => {
            ...
            // When
            const got = await cacheRetrieveOrRepopulate('identity-introspection-url-IDENTITYURL', async () => 'URL2', 60 * 1000, config);
            // Then
    ...
    test('derives the cached contents when the cache is not populated', async () => {
        await inTemporaryDirectory(async (cwd) => {
            ...
            const config = new LocalStorage({ cwd });
            // When
            const got = await cacheRetrieveOrRepopulate('identity-introspection-url-IDENTITYURL', async () => 'URL1', 60 * 1000, config);
            // Then
            expect(got).toEqual('URL1');
        });
    });
    test('re-derives the cached contents when the cache is outdated', async () => {
        await inTemporaryDirectory(async (cwd) => {
            ...
            config.set('cache', cacheValue);
            // When
            const got = await cacheRetrieveOrRepopulate('identity-introspection-url-IDENTITYURL', async () => 'URL2', 0, config);
            // Then
...
//# sourceMappingURL=conf-store.test.js.map