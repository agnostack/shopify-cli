...
describe('applying environments', async () => {
    const runTestInTmpDir = (testName, testFunc) => {
        test(testName, async () => {
            testResult = {};
            testError = undefined;
    ...
    function expectFlags(path, environment) {
        const envFlags = allEnvironments.environments && allEnvironments.environments[environment];
    ...
    runTestInTmpDir('does not apply a environment when none is specified', async (tmpDir) => {
        // Given
    ...
    runTestInTmpDir('applies a environment when one is specified', async (tmpDir) => {
        // Given
        const outputMock = mockAndCaptureOutput();
        outputMock.clear();
        // When
    ...
    runTestInTmpDir('searches up recursively from path by default', async (tmpDir) => {
        // Given
    ...
    runTestInTmpDir('prefers command line arguments to environment settings', async (tmpDir) => {
        // Given
        const outputMock = mockAndCaptureOutput();
        outputMock.clear();
        // When
    ...
    runTestInTmpDir('ignores the specified environment when it does not exist', async (tmpDir) => {
        // When
    ...
    runTestInTmpDir('does not apply flags irrelevant to the current command', async (tmpDir) => {
        // When
    ...
    runTestInTmpDir('throws when an argument of the incorrect type is provided', async (tmpDir) => {
        // When
    ...
    runTestInTmpDir('throws when exclusive arguments are provided', async (tmpDir) => {
        // When
    ...
    runTestInTmpDir('throws on negated booleans', async (tmpDir) => {
        // When
        await MockCommand.run(['--path', tmpDir, '--environment', 'environmentWithNegativeBoolean']);
        // Then
        expect(testError?.message).toMatch(/Environments can only specify true for boolean flags\. Attempted to set .+someBoolean.+ to false\./);
    });
    runTestInTmpDir('handles multiples correctly', async (tmpDir) => {
        // When
        await MockCommand.run(['--path', tmpDir, '--environment', 'environmentWithMultiples']);
        // Then
        expectFlags(tmpDir, 'environmentWithMultiples');
    });
    runTestInTmpDir('throws when exclusive arguments are provided when combining command line + environment', async (tmpDir) => {
        // When
        await MockCommand.run(['--path', tmpDir, '--environment', 'validEnvironment', '--someExclusiveString', 'stringy']);
        // Then
        expect(testError?.message).toMatch('--someBoolean=true cannot also be provided when using --someExclusiveString');
    });
    runTestInTmpDir('reports environment settings that do not match defaults', async (tmpDir) => {
        // Given
        const outputMock = mockAndCaptureOutput();
        outputMock.clear();
        // When
    ...
    runTestInTmpDir('reports environment settings that match defaults', async (tmpDir) => {
        // Given
        const outputMock = mockAndCaptureOutput();
        outputMock.clear();
        // When
    ...
    runTestInTmpDir('reports environment settings with masked passwords', async (tmpDir) => {
        // Given
        const outputMock = mockAndCaptureOutput();
        outputMock.clear();
        // When
