import { packageManagerUsedForCreating, addNPMDependenciesIfNeeded, installNodeModules, getDependencies, getPackageName, PackageJsonNotFoundError, checkForNewVersion, readAndParsePackageJson, findUpAndReadPackageJson, FindUpAndReadPackageJsonNotFoundError, usesWorkspaces, addResolutionOrOverride, writePackageJSON, getPackageManager, installNPMDependenciesRecursively, addNPMDependencies, } from './node-package-manager.js';
...
describe('findUpAndReadPackageJson', () => {
    test('returns the content of the package.json', async () => {
        await inTemporaryDirectory(async (tmpDir) => {
            ...
            expect(got.content).toEqual(packageJson);
        });
    });
    test("throws a FindUpAndReadPackageJsonNotFoundError error if it can't find a package.json", async () => {
        await inTemporaryDirectory(async (tmpDir) => {
            ...
            await mkdir(subDirectory);
            // When/Then
            await expect(() => findUpAndReadPackageJson(subDirectory)).rejects.toThrowError(FindUpAndReadPackageJsonNotFoundError(subDirectory));
        });
    });
});
...
describe('getPackageManager', () => {
    ...
    test("throws a FindUpAndReadPackageJsonNotFoundError error if it can't find a package.json", async () => {
        await inTemporaryDirectory(async (tmpDir) => {
            ...
            await mkdir(subDirectory);
            // When/Then
            await expect(() => getPackageManager(subDirectory)).rejects.toThrowError(FindUpAndReadPackageJsonNotFoundError(subDirectory));
        });
    });
});
...
//# sourceMappingURL=node-package-manager.test.js.map