...
describe('execCLI', () => {
    ...
    test('throws an exception when Ruby version requirement is not met', async () => {
        ...
        await expect(() => execCLI2(['args'])).rejects.toThrowError(`Ruby version \u001b[33m${rubyVersion}\u001b[39m is not supported`);
    });
    ...
    test('throws an exception when Bundler version requirement is not met', async () => {
        ...
        await expect(() => execCLI2(['args'])).rejects.toThrowError(`Bundler version \u001b[33m${bundlerVersion}\u001b[39m is not supported`);
    });
...
function mockBundledCLI2(cli2Directory, { windows }) {
    vi.mocked(getEnvironmentVariables).mockReturnValue({ SHOPIFY_CLI_BUNDLED_THEME_CLI: '1' });
...
async function mockEmbeddedCLI2(cli2Directory, { windows, existingWindowsDependency }) {
...
function mockPlatformAndArch({ windows }) {
    if (windows) {
...
async function createGemFile(cli2Directory, existingWindowsDependency) {
    const gemfilePath = joinPath(cli2Directory, 'Gemfile');
    let content = "source 'https://rubygems.org'\n";
    if (existingWindowsDependency)
        content = content.concat(`gem 'wdm', '>= ${MinWdmWindowsVersion}'`);
    await touchFile(gemfilePath);
    await appendFile(gemfilePath, content.concat('\n'));
}
function validateBundleExec(execSpy, gemFilePath, execPath = 'shopify', spinFqdn) {
...
async function validateGemFileContent(gemfilePath, { bundled, windows }) {
    ...
    if (bundled)
        expect(gemContent).toContain(`gem 'shopify-cli', '${RubyCLIVersion}'`);
